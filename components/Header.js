...
function Header({ placeholder }) {
   ...
   return (
      ...
      <header className="sticky top-0 z-50 grid grid-cols-3 bg-white shadow-md p-6 md:px-10">

         {/* Left-logo */}
         {/* router */}
         <div onClick={() => router.push("/")} className="left-logo relative flex items-center h-10 cursor-pointer my-auto">
            <Image
         ...
         {/* Middle-Search */}
         <div className="flex items-center md:border-2 rounded-full py-2 md:shadow-md hover:shadow-lg">
            <input
               value={searchInput}
               onChange={(e) => setSearchInput(e.target.value)}
               className="pl-5 bg-transparent flex-grow outline-none text-base text-gray-600 placeholder-gray-400" type="text" placeholder={placeholder || "Start your search"} />
         ...
         <div className="flex items-center space-x-4 justify-end">
            ...
            <div className="flex items-center space-x-2 border-2 rounded-full p-2">
               <MenuIcon className="h-6 text-sm rounded-full cursor-pointer text-gray-500" />
               <UserCircleIcon className="h-6 text-sm rounded-full cursor-pointer text-gray-500" />
            </div>
         </div>
