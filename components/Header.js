...
import Dropdown from "./DropdownMD";
...
function Header({ placeholder }) {
  ...
  return (
    <header className="sticky top-0 z-50 flex md:grid md:grid-cols-3 md:grid-cols-3 bg-white shadow-md p-4 md:px-8">
      {/* Big-Logo only in large screen */}
      {/* router */}
      <div
        onClick={() => router.push("/")}
        className="hidden lg:inline left-logo relative h-10 cursor-pointer my-auto"
      >
        <Image
      ...
      >
        <svg
          className="h-8 w-8 md:h-10 md:w-10 md:pt-2 cursor-pointer relative"
          layout="fill"
          objectFit="contain"
      ...
      <div className="flex items-center flex-grow border-2 rounded-full py-1 md:py-2 px-3 md:px-4 md:shadow-md hover:shadow-lg">
        <div className="flex items-center justify-center flex-grow cursor-pointer">
          <input
            ...
            className="bg-transparent md:flex-grow outline-none text-base md:text-base text-gray-600 placeholder-gray-600 focus:placeholder-gray-400 cursor-pointer"
            type="text"
            placeholder={placeholder || "Where are you going?"}
          />
        </div>
        <SearchIcon className="h-5 p-1 ml-1 text-white bg-red-400 rounded-full cursor-pointer md:hidden text-center" />
        <SearchIcon className="hidden h-6 ml-1 bg-red-400 text-white rounded-full p-1 cursor-pointer md:inline-flex " />
      </div>

      {/* Right */}
      <div className="flex items-center space-x-4 justify-end">
        <p className="hidden lg:inline p-2 px-3 font-medium rounded-full cursor-pointer text-gray-600 hover:text-gray-900 hover:bg-gray-200">
          Become a host
        </p>
      ...
      {searchInput && (
        <div className="flex flex-col col-span-3 mx-auto ">
          ...
          <div className="flex items-center border-b mb-4">
            <h2 className="text-2xl font-semibold pl-2 flex-grow">
              Number of Guests :
            </h2>
